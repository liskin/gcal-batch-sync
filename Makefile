...
.PHONY: test-pytest
## Invoke pytest tests (offline only)
test-pytest: $(VENV_DONE)
	$(VENV_PYTHON) -m pytest $(PYTEST_FLAGS) tests/

.PHONY: test-pytest-online
## Invoke pytest tests (offline + online)
test-pytest-online: export PYTEST_ONLINE=1
test-pytest-online: test-pytest
